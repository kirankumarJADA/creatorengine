...
const FailedJobs = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter((j) => {
      const haystack = [j.username, j.automationName, j.reason].filter(Boolean).join(' ').toLowerCase();
  ...
  if (!activeIgId) {
    return (
      <div>
        <PageHeader title="Failed Jobs" description="Automation runs that gave up after exhausting retries." />
        <EmptyState
          icon={AlertOctagon}
          title="No account selected"
          description="Select an Instagram account from the sidebar to view failed jobs."
        />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button variant="secondary" leftIcon={RefreshCw} onClick={fetchJobs}>Refresh</Button>}
      />
      ...
      {error && !isLoading && (
        <div className="mb-4 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 dark:border-red-500/30 dark:bg-red-500/10">
          ...
          <div className="min-w-0 flex-1 text-sm">
            <p className="font-semibold text-red-900 dark:text-red-200">Couldn't load failed jobs</p>
            <p className="mt-0.5 text-red-800 dark:text-red-300/90">{error}</p>
          </div>
          <Button size="sm" variant="secondary" onClick={fetchJobs}>Retry</Button>
        </div>
      )}
      ...
      <Card padded={false} className="overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            icon={AlertOctagon}
            title={query ? 'No matches' : error ? 'No failed jobs to show' : 'No failed jobs 🎉'}
            description={
              query
          ...
          <FailedJobsTable rows={paginated} pendingRetry={pendingRetry} pendingDelete={pendingDelete} onRetry={handleRetry} onDelete={handleDelete} />
        )}
        ...
        {rows.map((row, i) => {
          ...
          return (
            <motion.tr
              ...
              className={cn('border-b border-ink-100 last:border-0 transition-colors', 'hover:bg-ink-50/60 dark:border-ink-800 dark:hover:bg-ink-800/40', busy && 'opacity-60')}
            >
              ...
              <td className="max-w-sm truncate px-5 py-3.5 text-ink-600 dark:text-ink-400" title={row.reason || ''}>{row.reason || '—'}</td>
              <td className="px-5 py-3.5 font-mono text-ink-800 dark:text-ink-200">{row.attempts ?? 0}</td>
              ...
              <td className="px-5 py-3.5">
                <div className="flex items-center justify-end gap-1">
                  <IconButton aria-label="Retry this job" title="Retry" onClick={() => onRetry(row)} disabled={busy}>
                    {isRetrying ? <Loader2 size={16} className="animate-spin" /> : <RotateCcw size={16} />}
                  </IconButton>
                  <IconButton aria-label="Delete this job" title="Delete" onClick={() => onDelete(row)} disabled={busy}>
      ...
      <thead>
        <tr className="border-b border-ink-100 bg-ink-50/60 dark:border-ink-800 dark:bg-ink-900/60">
          {['Username', 'Automation', 'Reason', 'Attempts', 'Timestamp', 'Actions'].map((h, i) => (
            <th key={h} className={cn('px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-ink-500 dark:text-ink-400', i === 5 && 'text-right')}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
