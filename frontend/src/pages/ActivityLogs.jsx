...
const TRIGGER_LABEL = { COMMENT: 'Comment', DM: 'DM', STORY_REPLY: 'Story' };
...
const ActivityLogs = () => {
  ...
  const hasActiveFilters =
    query.trim() !== '' || statusFilter !== 'all' || automationFilter !== 'all' || dateFrom !== '' || dateTo !== '';

  if (!activeIgId) {
    return (
      <div>
        <PageHeader title="Activity Logs" description="What your automations have been doing." />
        <EmptyState
          icon={ScrollText}
          title="No account selected"
          description="Select an Instagram account from the sidebar to view activity logs."
        />
      </div>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button variant="secondary" leftIcon={RefreshCw} onClick={fetchLogs}>Refresh</Button>}
      />
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        ...
        <Dropdown label="Status" options={STATUS_FILTERS} value={statusFilter} onChange={(v) => { setStatusFilter(v); setPage(1); }} />
        <Dropdown label="Automation" options={automationOptions} value={automationFilter} onChange={(v) => { setAutomationFilter(v); setPage(1); }} />
        <div className="flex items-center gap-2">
          <input type="date" value={dateFrom} onChange={(e) => { setDateFrom(e.target.value); setPage(1); }} className="input min-w-0 flex-1" aria-label="From date" />
          <span className="text-ink-400">–</span>
          <input type="date" value={dateTo} onChange={(e) => { setDateTo(e.target.value); setPage(1); }} className="input min-w-0 flex-1" aria-label="To date" />
        </div>
      </div>
      ...
      {error && !isLoading && (
        <div className="mb-4 flex items-start gap-3 rounded-2xl border border-red-200 bg-red-50 p-4 dark:border-red-500/30 dark:bg-red-500/10">
          ...
          <div className="min-w-0 flex-1 text-sm">
            <p className="font-semibold text-red-900 dark:text-red-200">Couldn't load activity logs</p>
      ...
      <Card padded={false} className="overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            icon={ScrollText}
            title={hasActiveFilters ? 'No matches' : error ? 'No activity to show' : 'No activity yet'}
            description={
              hasActiveFilters
    ...
    <table className="w-full text-sm">
      ...
      <tbody>
        {rows.map((row, i) => (
          ...
          >
            <td className="px-5 py-3.5 font-mono text-ink-900 dark:text-ink-100">{row.username ? '@' + row.username : '—'}</td>
            <td className="px-5 py-3.5 text-ink-800 dark:text-ink-200">{row.automationName || <span className="italic text-ink-400">—</span>}</td>
            <td className="px-5 py-3.5 text-ink-700 dark:text-ink-300">{TRIGGER_LABEL[row.triggerType] || row.triggerType || '—'}</td>
            <td className="max-w-xs truncate px-5 py-3.5 text-ink-600 dark:text-ink-400" title={row.eventText || ''}>{row.eventText || '—'}</td>
            <td className="px-5 py-3.5"><Badge tone={STATUS_TONE[row.status] || 'neutral'} dot>{STATUS_LABEL[row.status] || row.status || '—'}</Badge></td>
            <td className="px-5 py-3.5 text-ink-500 dark:text-ink-400" title={row.timestamp || ''}>{row.timestamp ? formatRelative(row.timestamp) : '—'}</td>
          </motion.tr>
        ))}
      </tbody>
    </table>
...
const Pagination = ({ page, totalPages, total, pageSize, onChange }) => {
  ...
  return (
    <nav aria-label="Pagination" className="flex flex-col items-center justify-between gap-3 border-t border-ink-100 px-5 py-3 text-sm dark:border-ink-800 sm:flex-row">
      <p className="text-ink-500 dark:text-ink-400">Showing <span className="font-medium text-ink-800 dark:text-ink-200">{first}–{last}</span> of {total}</p>
