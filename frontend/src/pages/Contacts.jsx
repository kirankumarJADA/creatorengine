...
// Backend stores Contact.source as the EventType name (COMMENT/DM/STORY_REPLY).
// We render friendlier labels in the table and dropdown.
...
const Contacts = () => {
  ...
  const fetchContacts = async () => {
    ...
    try {
      ...
      // Service returns the unwrapped data array; defend against shape drift.
      setContacts(Array.isArray(data) ? data : []);
    } catch (err) {
      // Axios interceptor already toasts auth errors; just surface a banner.
  ...
  useEffect(() => {
    fetchContacts();
    // No dependencies — fetch once on mount, refresh button re-triggers manually.
    ...
    <div>
      ...
      <Card padded={false} className="overflow-hidden">
        {isLoading ? (
          ...
          <EmptyState
            icon={Users}
            title={query || sourceFilter !== 'all' ? 'No matches' :
                   error ? 'No contacts to show' : 'No contacts yet'}
            description={
      ...
      <tbody>
        {contacts.map((c, i) => (
          ...
          >
            ...
            <td
              className="px-5 py-3.5 font-mono text-xs text-ink-600 dark:text-ink-400"
              title={c.instagramUserId}
            >
            ...
            <td className="px-5 py-3.5 font-mono text-ink-800 dark:text-ink-200">
              {(c.totalTriggers ?? 0).toLocaleString()}
            </td>
            <td className="px-5 py-3.5 text-ink-700 dark:text-ink-300">
              {c.lastInteraction ? formatRelative(c.lastInteraction) : '—'}
            </td>
...
// Truncate long IG ids to keep the table compact; full id in title attr.
const truncateId = (id) => {
  if (!id) return '—';
...
// ─── Pagination (mirrors Automations.jsx for consistency) ───
