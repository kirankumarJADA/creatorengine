...
public class AutomationEngine {
    ...
    public void dispatch(String uid, WebhookEventDto event) {
        ...
        for (Automation automation : candidates) {
            ...
            // Pass the receiving account ID so the job knows which IG account to use
    ...
    private void handleFollowGateCompletion(String uid, WebhookEventDto event) {
        ...
        // Try new per-account path first, fall back to legacy
        String igAccountId = event.receivingAccountId();
        Optional<Automation> automationOpt = igAccountId != null
    ...
    public void processJob(AutomationJob job) {
        ...
        Optional<Automation> automationOpt = igAccountId != null
                ? automationRepository.findById(job.uid(), igAccountId, job.automationId())
                : automationRepository.findById(job.uid(), job.automationId());

        if (automationOpt.isEmpty()) {
            log.info("Skipping job {} - automation {} no longer exists", job.jobId(), job.automationId());
            return;
        }

        Automation automation = automationOpt.get();
        if (!automation.getEnabled()) {
            log.info("Skipping job {} - automation {} was disabled", job.jobId(), job.automationId());
            return;
        }

        // Load the specific IG account that received the webhook event
        InstagramAccount account = igAccountId != null
                ? instagramAccountService.findByIgId(job.uid(), igAccountId).orElse(null)
        ...
        if (actions.isEmpty()) {
            if (automation.getPublicReplyEnabled() && job.event().type() == EventType.COMMENT) {
        ...
        for (int i = job.actionIndex(); i < actions.size(); i++) {
            ...
            if (action.getType() == ActionType.DELAY) {
                ...
                Duration wait = secs == null ? Duration.ZERO : Duration.ofSeconds(Math.max(0, secs));
                queue.enqueueDelayed(job.advanceTo(i + 1), wait);
            ...
            if (isSend && rateKey != null && !rateLimitService.tryAcquire(rateKey)) {
                ...
                queue.enqueueDelayed(job.withActionIndex(i).withAttempt(currentAttempt), backoff);
            ...
            boolean userVisibleSuccess = result.messageSent() || isSaveContactSuccess(result, action);
            ...
            if (retryPolicy.isRetryable(result)) {
                ...
                if (nextAttempt <= retryPolicy.maxAttempts()) {
                    ...
                    queue.enqueueDelayed(
                            job.withActionIndex(i).withAttempt(nextAttempt).withLastError(result.error()), delay);
    ...
    private void runFollowGateAsk(AutomationJob job, Automation automation, InstagramAccount account) {
        ...
        try {
            ...
            log.error("Follow-gate ask threw for automation {}: {}", automation.getId(), ex.getMessage(), ex);
            result = ExecutionResult.failed(null, "Follow-gate ask exception: " + ex.getMessage(), null);
        }
        ...
        if (retryPolicy.isRetryable(result)) {
            ...
            if (nextAttempt <= retryPolicy.maxAttempts()) {
                Duration delay = retryPolicy.backoffFor(nextAttempt);
                queue.enqueueDelayed(job.withAttempt(nextAttempt).withLastError(result.error()), delay);
                return;
            }
            deadLetterService.record(job, automation, "Follow-gate ask max retries: " + result.error());
            return;
        }

        deadLetterService.record(job, automation, "Follow-gate ask failed: " + result.error());
    }
    ...
    private void runPublicReplyOnly(AutomationJob job, Automation automation, InstagramAccount account) {
        ...
        try {
            ...
            log.error("Public-reply-only threw for automation {}: {}", automation.getId(), ex.getMessage(), ex);
            result = ExecutionResult.failed(null, "Public-reply-only exception: " + ex.getMessage(), null);
        }
        ...
        if (result.messageSent()) {
            ...
            log.warn("Public-reply-only failed for automation {}: {}", automation.getId(), result.error());
        }
    }
